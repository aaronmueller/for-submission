...
# like a seq2seq RNNDecoder, but with hacks for the
# context layer and 
class HREDRNNDecoder(nn.Module):
    """
    Recurrent decoder module.
    ...
    def forward(self, xs, encoder_output, context_vector, incremental_state=None):
        """
        Decode from input tokens.
        ...
        :param encoder_output: output from RNNEncoder. Tuple containing
            (enc_out, enc_hidden, attn_mask) tuple. 
        :param context_vector: context vector at previous step 
            shape (bsz x contextsize)
        :param incremental_state: most recent hidden state to the decoder.
            If None, the hidden state of the encoder is used as initial state,
            and the full sequence is computed. If not None, computes only the
            next forward in the sequence.
        ...
        # need to resize context vector for later use
        context_vector = context_vector.unsqueeze(1)
        context_vector = (torch.cat((context_vector, context_vector), 1),
                          torch.cat((context_vector, context_vector), 1))

        if incremental_state is not None:
            hidden = _transpose_hidden_state(incremental_state)
            xs = xs[:, -1:]
        else:
            # Use the context vector state as our start state
            hidden = _transpose_hidden_state(context_vector)
        ...
        if self.attn_time != 'post':
            # looks like this already works? context vector not needed here
            output, new_hidden = self.rnn(xes, hidden)
        else:
            # uh oh, post attn, we need run through one at a time, and do the
            # attention modifications
